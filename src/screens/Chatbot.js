  ...
  const submitHandler = e => {
    ...
    axios.post('https://n9i31tpdha.execute-api.us-east-1.amazonaws.com/v1/chatbot', newMessage).then(value => {
        ...
        for (const slot in parsed.sessionState.intent.slots) {
            ...
            if (parsed.sessionState.intent.slots[slot].value && !parsed.sessionState.intent.slots[slot].value.interpretedValue) {
                parsed.sessionState.intent.slots[slot] = {}
            }
        }
        ...
        if (parsed.sessionState.intent.state === 'Fulfilled') {
            ...
            setMetadata({...metadata, sessionState: {'intent': {'name': 'FallbackIntent', 'slots': {}}, 'sessionAttributes':parsed.sessionState.sessionAttributes}, sessionStarted: true})
        }
        else {
